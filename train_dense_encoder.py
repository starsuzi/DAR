...
from dar.models import init_biencoder_components
from dar.models.biencoder import BiEncoder, BiEncoderNllLoss, BiEncoderBatch, BiEncoderMixupNllLoss
from dar.options import (
    setup_cfg_gpu,
    set_seed,
...
from dar.utils.conf_utils import BiencoderDatasetsCfg
from dar.utils.data_utils import (
    ShardedDataIterator,
    Tensorizer,
    MultiSetDataIterator,
)
from dar.utils.dist_utils import all_gather_list
from dar.utils.model_utils import (
...
class BiEncoderTrainer(object):
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            # get the token to be used for representation selection
            from dar.data.biencoder_data import DEFAULT_SELECTOR
...
def _do_biencoder_fwd_pass(
    ...
    #(TODO) Loss function selection
    loss_function_type = cfg.train.loss_function_type
    
    if loss_function_type == 'nll':
        loss_function = BiEncoderNllLoss()
    elif loss_function_type == 'mixup_nll':
        loss_function = BiEncoderMixupNllLoss()

    #(TODO) Perturbation with dropout
    dropout = cfg.train.dropout
    if dropout == True:
        lst_local_ctx_vectors = [local_ctx_vectors]
        drop_layer = nn.Dropout(0.1)
        
        for i in range(0,7):
            dropout_local_ctx_vectors = drop_layer(local_ctx_vectors)
            lst_local_ctx_vectors.append(dropout_local_ctx_vectors)
        
        loss = 0
        is_correct = 0
        for idx, local_ctx_vectors in enumerate(lst_local_ctx_vectors):
            l, is_c = _calc_loss(
                cfg,
                loss_function,
                local_q_vector, 
        ...
        loss = loss / len(lst_local_ctx_vectors)
