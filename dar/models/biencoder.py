...
from dar.data.biencoder_data import BiEncoderSample
from dar.utils.data_utils import Tensorizer
from dar.utils.model_utils import CheckpointState
...
class BiEncoderMixupNllLoss(object):
    def calc(
        self,
        q_vectors: T,
        ctx_vectors: T,
        ...
        """
        Computes mixup nll loss for the given lists of question and ctx vectors.
        Note that although hard_negative_idx_per_question in not currently in use, one can use it for the
        loss modifications. For example - weighted NLL with different factors for hard vs regular negatives.
        :return: a tuple of loss value and amount of correct predictions per batch
        """
        ...
        ### For Mixup ###
        #(TODO:) making mix-up for obtained similarites between a query and document.
        # Get the number of samples for mixup
        B = scores.size(0)     

        pos_idx = torch.tensor(positive_idx_per_question).to(scores.device)
        pos_scores = scores[torch.arange(B), pos_idx] 

        lst_neg_scores = []
        for i, pos_idx in enumerate(positive_idx_per_question):
            lst_row_scores = scores[i].tolist()
            lst_row_scores.pop(pos_idx)
            lst_neg_scores.append(lst_row_scores)
        neg_scores = torch.tensor(lst_neg_scores, device=scores.device, requires_grad=True)

        sum_mixed_loss = 0
        for i in range(len(neg_scores[0])):
            pos_data = pos_scores
            neg_data = neg_scores[:,i]
            norm_neg_data = neg_data - (torch.cat((neg_data, pos_data), dim = 0)).mean()
            norm_pos_data = pos_data - (torch.cat((neg_data, pos_data), dim = 0)).mean()

            label_mix = torch.from_numpy(np.random.uniform(0, 1, size=(B))).float().to(scores.device)
            data_mix = label_mix * norm_pos_data + (1-label_mix) * norm_neg_data
            data_mix /= 10

            data_mix_sigmoid = torch.sigmoid(data_mix)
            mixed_loss = F.binary_cross_entropy(data_mix_sigmoid, label_mix)

            sum_mixed_loss = sum_mixed_loss + mixed_loss
            
        sum_mixed_loss = sum_mixed_loss/(len(neg_scores[0]))

        if loss_scale:
            sum_mixed_loss.mul_(loss_scale)
        
        final_loss = 0.9 * loss + 0.1 * sum_mixed_loss

        logger.info("  Original Loss:{} / Mixup Loss:{} / Final Loss:{}  ".format(loss, sum_mixed_loss, final_loss))

        return final_loss, correct_predictions_count
...
def _select_span_with_token(
    ...
    if id not in query_tensor:
        ...
        if token_indexes.size(0) > 0:
            ...
            from dar.models.reader import _pad_to_len
